...
function obtenerEnvioPorPatentCamion() {
  console.log("obtenerEnvioPorPatentCamion");

  const patente = document.getElementById("FiltroPatente").value;

  urlConFiltro = `http://localhost:8080/envios/${patente}`; //ver que url colocariamos
...
function exitoObtenerCamionesPorPatente(data) {
  ...
  row.innerHTML = ` 
                ...
                <td>${data.fecha_ultima_actualizacion}</td>
                <td>${data.patente_camion}</td>
                <td>
                    <table>
                        <tr>
                            <th>Ciudad</th>
